...
EXAMPLE="$ROOT/config/kit/deploy.cf.example.toml"
LOCAL="$ROOT/config/kit/deploy.cf.local.toml"
PRINT=0
...
for arg in "$@"; do
  case "$arg" in
    --print) PRINT=1 ;;
    -h|--help)
      echo "Usage: bash scripts/kit/check-deploy-cf-config.sh [--print]"
      echo "  Requires config/kit/deploy.cf.local.toml (copy from deploy.cf.example.toml)."
      exit 0
      ;;
  esac
done
...
if [[ ! -f "$EXAMPLE" ]]; then
  echo "error: missing tracked template: config/kit/deploy.cf.example.toml" >&2
  exit 1
fi

if [[ ! -f "$LOCAL" ]]; then
  echo "error: missing local deploy profile: config/kit/deploy.cf.local.toml" >&2
  echo "  cp config/kit/deploy.cf.example.toml config/kit/deploy.cf.local.toml" >&2
  echo "  # then fill cf_account_id, zone, hostnames, D1/R2 names" >&2
  echo "  See docs/kit/deploy-cloudflare.md" >&2
  exit 1
fi
...
if [[ "$FAIL" -ne 0 ]]; then
  echo "check-deploy-cf-config: FAIL — fix config/kit/deploy.cf.local.toml" >&2
  exit 1
fi
...
echo "Next: wrangler whoami  # must match cf_account_id — docs/kit/deploy-cloudflare.md §2"
exit 0
