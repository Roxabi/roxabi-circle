...
export function MagicLinkEmail(props: { to: string; magicUrl: string; expiresHint?: string }) {
  ...
  const subject = 'Sign in — GOSILEX kit'
  const exp = props.expiresHint ? props.expiresHint.replace(/[\r\n]/g, '') : 'about 5 minutes'
  const text = [
    'You requested a sign-in link for GOSILEX Kit.',
    `Sign-in link: ${props.magicUrl}`,
    `This link expires in ${exp}.`,
    'If you did not request this, ignore this email.',
  ].join('\n')
  const html = `<p>You requested a sign-in link for <strong>GOSILEX Kit</strong>.</p><p><a href="${safeUrl}">Sign in</a></p><p>This link expires in ${escapeHtml(exp)}.</p><p>If you did not request this, ignore this email.</p>`
