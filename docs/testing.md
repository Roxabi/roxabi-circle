...
| **CP-KIT-AUTH** | products import `createBetterAuth` from `@kit/auth/factory` (signup default off, secure cookies, magic/reset EmailPort, secret denylist); env helpers fail-closed | `packages/auth` unit (`better-auth-env`, `auth-email`, `first-session-hook`, factory options) · example-api magic-link / audit dogfood |
