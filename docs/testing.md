...
| **CP-KIT-SCHEMA** | kit D1 modules identified by catalog id + sha256; product appends `NNNN_kit_<id>.sql` and never reuses an applied `NNNN`; adopt matches raw clones; mutated published id fails; wrangler `migrations_dir` never resolves under `packages/` (live-tree harness checks exit 0 + `OK`, not a pinned file count — products add `apps/<product>-*` wrangler files) | `bun run test:kit-schema-sync` · `scripts/kit-schema-sync.sh` · `bun run wrangler-migrations:check` · `bun run test:wrangler-migrations` · [ADR-0008](./architecture/adr/0008-kit-schema-identity-product-compose.md) (in `validate:full`) |
