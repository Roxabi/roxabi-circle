...
| **CP-KIT-AUTH** | products import `createBetterAuth` from `@kit/auth` (factory + magic/reset EmailPort + first_login hook wrap); env helpers fail-closed; no copy of `example-api` glue | `packages/auth` unit (`better-auth-env`, `auth-email`, `first-session-hook`, factory) · example-api magic-link / audit dogfood |
