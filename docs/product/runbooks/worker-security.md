...
| `POST /internal/github-digest` | **`X-Ops-Secret`** | retired GitHub scrape-post — returns `{ skipped: "disabled" }`, no I/O |
...
| Cron | `*/15` Gateway safety net only. Former `30 10` + `30 11` UTC GitHub digest triggers are **removed**; leftover ticks no-op in code |
