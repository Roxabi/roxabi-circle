...
| Discord | Ed25519 `/interactions` · `/apply` scaffold · appeal tickets · Gateway `#github-to-watch` / `#news-actu` · `@Lyra` mention → `LYRA_GROK_WEBHOOK_URL` · `#github-to-watch` repo URL → `LYRA_GITHUB_WATCH_WEBHOOK_URL` |
...
| `#daily-digest` | **Lyra / Veilleur** one message (news + repos). Worker GitHub scrape-post is **off**. Gateway: bots-only top-level · leftover cron / ops POST no-op |
