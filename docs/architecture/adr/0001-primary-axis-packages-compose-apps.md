...
Dual mission monorepo: Chemin A Cloudflare SaaS kit (P0) and future silex-share product apps (P1). Without a primary axis, implementers risk N×M drift (auth/errors/storage forked per app, or product domain leaking into packages).

## Axes

1. **Platform concern** — `packages/*` (`@gosilex/*`)
...
**Test:** Adding a second product creates `apps/<name>-*` and imports `@gosilex/*` — it does not copy AppError/auth/db/storage stacks, and does not add `packages/share-*`.
...
- Product schemas (e.g. artefacts) live in product apps, not `@gosilex/db`

## Anti-pattern signals

- Product markers under `packages/` (`share/{slug}`, `private_key` product mode, `share_publish`, Shlink)
- Local `class AppError` under `apps/` instead of `@gosilex/core`
