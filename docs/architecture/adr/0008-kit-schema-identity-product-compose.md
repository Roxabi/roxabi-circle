...
related:
  ...
  - docs/playbooks/start-product.md
  - docs/kit-schema-sync.md
---
...
Operator how-to: [`docs/kit-schema-sync.md`](../../kit-schema-sync.md).
...
| Sync kit SQL via `scripts/kit-schema-sync.sh` | Hand-copy `example-api/migrations` then domain at 0009 |
...
Honest debt: kit SQL still lives in the dogfood app, not in packages. This ADR does **not** move files. Promoting sketches → applied SSoT is a follow-up ADR. `@kit/auth` does **not** export `./migrations/*` — applying `packages/auth/migrations` is a hard error.

### D4 — Sync is append-only

```text
scripts/kit-schema-sync.sh --app apps/<product>-api [--modules core|all|<sets>]
```
