...
### D1 — Symmetric namespaces under `config/` · `scripts/` · `docs/`
...
Under `config/` · `scripts/` · `docs/`, every tracked path must be under `kit/` or `product/` (or a documented exception path listed in zones). Unclassified → fail.
