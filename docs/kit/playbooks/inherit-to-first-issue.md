...
Short path after [`start-product.md`](./start-product.md). GitHub Fork remains **DENY** (fork network: PRs default to the kit parent; private/deletion stay coupled).

1. Product repo whose `origin` is the product + `upstream` fetch-only + `config/product/inheritance.json` already committed (operator kit parent URL).
