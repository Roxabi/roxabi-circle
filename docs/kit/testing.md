...
| **CP-EXTRACT** | structural extractability: required tree, kit app allowlist, banlist, import graph, orphan packages, ADRs, **residency** (no kit-generic `sqliteTable` / local org policy in apps), **temp compose proof** (`tsc` + org-scoped Hono 200/404 via `@kit/auth` repos — inverse greps alone are insufficient) | `scripts/kit/extract-dry-run.sh` · `bun run test:extract-dry-run` |
