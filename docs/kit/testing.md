...
| `--config=scripts/kit/trufflehog-detectors.yaml`, no such flag | `unverified_secrets: 1` → exit 183 |
...
# See docs/kit/templates/product-validate.example.sh + product-ci.example.yml
```
...
Floors are enforced by Vitest (`packages/config/vitest-coverage.mjs` + per-package `vitest.config.ts`). Runner: `scripts/kit/test-coverage.sh`.
...
| **`scripts/kit/*` / `tools/*`** | Architecture gates (kit-bar script names live in root `package.json`, not here) | Domain behaviour |
...
- **Local one-shot (cold start):** `bun run test:e2e:ci` (`scripts/kit/e2e-ci.sh` — migrate, seed, start API+web, Chromium smoke, teardown).
...
| **CP-BAN** | no product-share strings in packages / examples | `scripts/kit/check-banned-strings.sh` |
| **CP-EXTRACT** | structural extractability: ADR-0009 D5 tree identity (`scripts/kit/resolve-tree-identity.mjs`); kit example-app allowlist **kit trees only**; product apps (complement of examples, not product-name enum) allowed on **product** trees; harness audit via `EXTRACT_MODE` + `EXTRACT_HARNESS_SENTINEL` only; banlist, import graph, orphan packages, ADRs, **residency**, **temp compose proof** unchanged | `scripts/kit/extract-dry-run.sh` · `bun run test:extract-dry-run` |
| **CP-ZERO-EDIT** | product consumers do not dual-edit kit paths; design overrides preferred; exceptions time-boxed + ticketed | `scripts/kit/check-zero-edit-zones.sh` · `config/kit/zero-edit-zones.json` · [`product-consumer-contract.md`](./product-consumer-contract.md) |
| **CP-DENY** | multi-hop deny-upstream: kit origin no-op; product blocks remote name `upstream`, kit URL substring, and product/env-extended chassis substrings; weaken name-guard fails harness | `bun run test:deny-upstream` · `scripts/kit/test-deny-upstream.sh` · `scripts/kit/deny-upstream-push.sh` (in `validate:full`) |
| **CP-FILE-LENGTH** | product file-length register: `apps/<product>-{api,web,mcp}/` only; kit-path / no-cap / glob / `..` / dup fail; missing file is a no-op | `bun run test:file-length` · `scripts/kit/test-file-length.sh` (in `validate:full`) |
| **CP-DOGFOOD** | consumer contract self-sim: kit→mirror→product chain; inheritance base only (#103 stale `upstream/main` ignored); dual-edit fail; `no_push`; deny-upstream on product | `bun run test:dogfood-zero-edit` · `scripts/kit/dogfood-zero-edit.sh --self-sim` (in `validate:full`) |
| **CP-KIT-SCHEMA** | kit D1 modules identified by catalog id + sha256; product appends `NNNN_kit_<id>.sql` and never reuses an applied `NNNN`; adopt matches raw clones; mutated published id fails; wrangler `migrations_dir` never resolves under `packages/` (live-tree harness: exit 0 + `check-wrangler-migrations-dir: OK` + ≥1 wrangler file — not a pinned count; product-shaped extra files still fail-closed on a sketch dir) | `bun run test:kit-schema-sync` · `scripts/kit/kit-schema-sync.sh` · `bun run wrangler-migrations:check` · `bun run test:wrangler-migrations` · [ADR-0008](./architecture/adr/0008-kit-schema-identity-product-compose.md) |
...
| **CP-IMPORT** | static R1–R4 import edges after kit + optional product exemptions (`config/product/import-boundary-exemptions.txt`, product-app paths only); self-test plants edges in temp tree | `bun run import-boundary` · `scripts/kit/check-import-boundaries.ts` · `bun run test:import-boundary` (in `validate:full`) |
...
| **CP-SEMCTX-WORKING** | PR head tree: `.semctx/working/` has only `.gitkeep` (`git ls-tree`, not runner workdir) | `scripts/kit/check-semctx-working-empty.sh` · `lefthook run pr` · CI job `semctx-working-empty` (merge-on-green required) · `bun run test:semctx-working-empty` (in `validate:full`) |
...
| `debt:check` / **CP-DEBT** | Line comments `biome-ignore` / `@ts-expect-error` / `@ts-ignore` under `apps|packages` tagged with `DEBT:<slug>`; untagged + stale-without-pin reported (warn default → exit 0; fail via env); self-test covers untagged matrix **and** expiry (stale fail/warn, pin exempt) | That warn mode “manages” debt; line-level blame age; open GitHub issue existence for `#N` pins; `biome.json` overrides; suppressions in `scripts/kit/`/`tools/`; full factory debt registry |
...
| `scripts/kit/test-coverage.sh` | monorepo runner |
| `scripts/kit/print-coverage-summary.mjs` | table after run |
