...
| Kit CD (`scripts/cf-builds/*`) | Refuses any branch ≠ `main` · **always** `--env production`. Copying it onto git `staging` writes **production** D1 |
...
`scripts/cf-builds/*` cannot deploy staging. Do not copy them onto a `staging` branch.
...
Zero-edit: **do not** patch `apps/example-*/wrangler.toml` or `scripts/cf-builds/*`.
...
7. Account / zone: `config/deploy.cf.local.toml` (gitignored).
...
| Copy `scripts/cf-builds/*` onto git `staging` | Product deploy command with explicit `--env staging` |
