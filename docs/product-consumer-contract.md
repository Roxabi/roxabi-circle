...
Auth glue (ADR-0008 D6): import `createBetterAuth` from `@kit/auth/factory`, tables from `@kit/auth/schema`, env helpers from `@kit/auth`. Do **not** copy or dual-edit `apps/example-api/src/lib/better-auth.ts` as the factory.
